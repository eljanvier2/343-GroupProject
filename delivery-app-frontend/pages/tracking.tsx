...
const Tracking = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl font-bold">Track Your Package</CardTitle>
          <div className="mt-2">
          ...
          <div className="space-y-6">
            {trackingData.updates.map((update, index) => (
              <div key={index} className="flex gap-4">
                <div className={`mt-1 ${update.isCurrentStatus ? 'text-blue-500' : 'text-gray-400'}`}>
                  {getStatusIcon(update.status)}
                </div>
                <div className="flex-1">
                  <div className={`font-semibold ${update.isCurrentStatus ? 'text-blue-500' : ''}`}>
                    {update.status}
                  </div>
